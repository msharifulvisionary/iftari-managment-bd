...
import { Plus, Trash2, Edit2, Download, Calendar, User, DollarSign, ShoppingBag, FileDown } from 'lucide-react';
...
export const Expense: React.FC = () => {
  ...
  const [formData, setFormData] = useState({ date: '', shopper: '', amount: '' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const expenseRef = ref(db, `expenses/${currentUser.uid}`);
    if (editingId) {
      await update(ref(db, `expenses/${currentUser.uid}/${editingId}`), {
        date: formData.date,
        shopper: formData.shopper,
    ...
    setFormData({ date: '', shopper: '', amount: '' });
  };

  const handleEdit = (expense: Expense) => {
    setFormData({ date: expense.date, shopper: expense.shopper, amount: expense.amount.toString() });
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      pdf.save(`Iftar_Expense_${profile?.month}_${profile?.year}.pdf`);
    } finally {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      link.download = `Iftar_Expense_${profile?.month}_${profile?.year}.png`;
      link.href = dataUrl;
      link.click();
    } finally {
      tableRef.current.classList.remove('exporting-mode');
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">বাজার খরচ</h1>
          <p className="text-slate-600 dark:text-slate-400 mt-1">প্রতিদিনের ইফতার বাজারের খরচের হিসাব</p>
        </div>
        ...
        <div className="lg:col-span-1">
          <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 shadow-sm border border-slate-100 dark:border-slate-700 sticky top-24">
            <h2 className="text-lg font-bold text-slate-900 dark:text-white mb-6 flex items-center gap-2">
              ...
              {editingId ? 'খরচ আপডেট করুন' : 'নতুন খরচ যোগ করুন'}
            </h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    type="date"
                    required
                    className="pl-10 block w-full rounded-xl border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-700/50 text-slate-900 dark:text-white focus:ring-rose-500 focus:border-rose-500 sm:text-sm py-3 px-4 transition-colors"
                    value={formData.date}
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">বাজারকারীর নাম</label>
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                  ...
                  <input
                    ...
                    onChange={(e) => setFormData({ ...formData, shopper: e.target.value })}
                  />
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">খরচের পরিমাণ</label>
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
              ...
              <button
                type="submit"
                className="w-full flex justify-center py-3 px-4 border border-transparent text-sm font-medium rounded-xl text-white bg-rose-600 hover:bg-rose-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-rose-500 transition-all mt-6"
              >
              ...
              {editingId && (
                <button
                  type="button"
                  onClick={() => { setEditingId(null); setFormData({ date: '', shopper: '', amount: '' }); }}
        ...
        <div className="lg:col-span-2">
          <div ref={tableRef} className="bg-white dark:bg-slate-800 rounded-3xl p-6 shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden">
            ...
            <div className="text-center mb-8 pb-6 border-b border-slate-200 dark:border-slate-700">
              <h2 className="text-2xl font-bold text-rose-600 dark:text-rose-400 mb-2">{profile?.messName}</h2>
              <p className="text-slate-600 dark:text-slate-400 font-medium text-lg">ইফতার বাজার খরচ - {profile?.month} {profile?.year}</p>
              ...
              <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
                <thead>
                  <tr>
                    ...
                    <th className="px-6 py-4 text-left text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">বাজারকারী</th>
                    <th className="px-6 py-4 text-right text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">খরচের পরিমাণ</th>
                    ...
                    expenses.map((expense, index) => (
                      <tr key={expense.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-900 dark:text-white font-medium">
                          {formatBengaliDate(expense.date)}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-900 dark:text-white font-medium">{expense.shopper}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-rose-600 dark:text-rose-400 font-bold text-right">
                          <span className="whitespace-nowrap">৳ {expense.amount.toLocaleString('bn-BD')}</span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium action-col">
                          <button onClick={() => handleEdit(expense)} className="text-blue-600 hover:text-blue-900 dark:text-blue-400 dark:hover:text-blue-300 mr-4">
                ...
                <tfoot>
                  <tr className="bg-slate-50 dark:bg-slate-900/50">
                    ...
                    <td className="px-6 py-4 text-right text-lg font-bold text-rose-600 dark:text-rose-400 whitespace-nowrap">
